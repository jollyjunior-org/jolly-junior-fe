...
```env
# JollyJuniors/.env
NEXT_PUBLIC_AUTH_API_BASE_URL=http://localhost:8001/api/v1
NEXT_PUBLIC_ADMIN_API_BASE_URL=http://localhost:8002/api/v1
NEXT_PUBLIC_PUBLIC_API_BASE_URL=http://localhost:8003/api/v1
```

```typescript
// Example store actions in useShopStore.ts
fetchProducts: async () => {
  const res = await axios.get(`${process.env.NEXT_PUBLIC_PUBLIC_API_BASE_URL}/products`);
...
login: async (email, password) => {
  ...
  const res = await axios.post(`${process.env.NEXT_PUBLIC_AUTH_API_BASE_URL}/auth/login`, form);
