  ...
  return (
    <div className="fixed inset-0 z-[90] bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-lg p-6 shadow-xl relative max-h-[92vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-black text-[#5A5A40] mb-1">Share your happy moment</h2>
        ...
        {done && (
          <div className="py-10 text-center space-y-2">
            ...
            <p className="font-bold text-[#5A5A40]">Thank you, parent!</p>
            <button
              type="button"
              onClick={close}
              className="mt-2 px-4 py-2 bg-[#5A5A40] text-white text-xs font-bold rounded-full cursor-pointer"
            >
              Back to shop
            </button>
          </div>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((n) => (
                ...
                >
                  <Star
                    className={`w-5 h-5 ${n <= form.rating ? 'fill-[#FFB347] text-[#FFB347]' : 'text-slate-300'}`}
                  />
                </button>
              ))}
            </div>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-2.5 bg-[#5A5A40] text-white text-sm font-bold rounded-xl cursor-pointer flex items-center justify-center gap-2"
            >
