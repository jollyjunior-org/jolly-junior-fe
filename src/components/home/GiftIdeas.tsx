                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-extrabold transition-all cursor-pointer ${
                    selectedAge === age
                      ...
                      : 'bg-[#FFFDF8] hover:bg-[#F5F2ED] text-[#5A5A40] border border-[#F5F2ED]'
                  }`}
                >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-extrabold transition-all cursor-pointer ${
                    selectedBudget === b.id
                      ? 'bg-[#FFB347] text-white shadow-xs'
                      : 'bg-[#FFFDF8] hover:bg-[#F5F2ED] text-[#5A5A40] border border-[#F5F2ED]'
                  }`}
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {filteredGifts.length > 0 ? (
            ...
            <div className="col-span-full py-10 text-center bg-white rounded-2xl p-6">
              ...
              <p className="text-xs font-bold text-[#5A5A40]">No products match this exact combination.</p>
              <button
                onClick={() => {
                  setSelectedAge('0-6M');
                  setSelectedBudget('all');
                }}
                className="mt-3 px-4 py-1.5 bg-[#F5F2ED] text-[#5A5A40] text-xs font-bold rounded-full cursor-pointer"
              >
                Reset Filters
              </button>
            </div>
