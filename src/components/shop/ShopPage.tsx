...
export const ShopPage: React.FC = () => {
  ...
  const renderActiveChips = (isMobile = false) => {
    ...
    return (
      <>
        {filter.saleKey && (
          <span className={`${chipClass} bg-[#F47C4C]/15 text-[#F47C4C]`}>
            Sale: {activeSale?.title || filter.saleKey}
            <X className="w-3 h-3 cursor-pointer ml-1" onClick={() => setFilter({ saleKey: null })} />
          </span>
        )}
        {[...selectedSlugs].map((slug) => (
          <span key={slug} className={`${chipClass} bg-[#FFFDF7] text-[#0798AE] border border-[#D9F1F5]`}>
        ...
        {filter.searchQuery && (
          <span className={`${chipClass} bg-[#D9F1F5] text-[#0798AE]`}>
            &quot;{filter.searchQuery}&quot;
            <X className="w-3 h-3 cursor-pointer ml-1" onClick={() => setFilter({ searchQuery: '' })} />
          </span>
        )}
      </>
    );
  ...
  // Reusable filter UI content for both desktop sidebar and mobile modal
  const FilterContent = () => (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-5">
      ...
      {/* Mobile Top Bar (OLX style) */}
      <div className="flex lg:hidden items-center gap-2 overflow-x-auto no-scrollbar pb-1">
        <button
          onClick={() => setIsMobileFilterOpen(true)}
          className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-[#E2E8F0] text-xs font-bold text-[#263238] shadow-xs cursor-pointer hover:bg-slate-50 transition-colors"
        >
          <Filter className="w-3.5 h-3.5 text-[#0798AE]" />
          <span>Filters</span>
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="hidden lg:block space-y-6 bg-white p-5 rounded-xl border border-[#F1F5F9] shadow-xs h-fit z-40">
          <div className="flex items-center justify-between pb-3 border-b border-[#F1F5F9]">
            <div className="flex items-center gap-2 text-sm font-black text-[#263238]">
